 ...
 * ペイクイック
 *
 * @ORM\Table(name="plg_remise_payment4_payquick")
 * @ORM\Entity(repositoryClass="Plugin\RemisePayment42\Repository\PayquickRepository")
 */
class Payquick
{
    /**
     ...
     * @return string
     */
    public function getDispCardParts()
    {
        if (empty($this->card_parts)) return $this->card_parts;

        $card = '';
        $idx = strpos($this->card_parts, '-');
        if ($idx !== false)
        {
            $card .= substr($this->card_parts, 0, $idx);
            $card .= "******";
            $card .= substr($this->card_parts, $idx+1);
        }
        else
        {
            for ($i = 0 ; $i < 16 - strlen($this->card_parts); $i++)
            {
                $card .= "*";
            }
            $card .= $this->card_parts;
        }
     ...
     * @return string
     */
    public function getDispExpire()
    {
        if (empty($this->expire)) return $this->expire;

        if (strlen($this->expire) >= 4)
        {
            return substr($this->expire, 0, 2) . trans('remise_payment4.common.label.month')
                . " " . substr($this->expire, 2) . trans('remise_payment4.common.label.year');
        }
