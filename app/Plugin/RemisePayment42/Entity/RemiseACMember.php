 ...
 * ルミーズ定期購買メンバー情報
 *
 * @ORM\Table(name="plg_remise_payment4_remise_ac_member")
 * @ORM\Entity(repositoryClass="Plugin\RemisePayment42\Repository\RemiseACMemberRepository")
 */
class RemiseACMember
{
    /**
     * @var string
     *
     ...
     * @return string
     */
    public function getCardpartsProcessing()
    {
        if (empty($this->getCardparts())) return "";

        $card = '';
        $idx = strpos($this->getCardparts(), '-');

        if ($idx !== false) {
            $card .= substr($this->getCardparts(), 0, $idx);
            $card .= "******";
            $card .= substr($this->getCardparts(), $idx+1);
        } else {
            for ($i = 0 ; $i < 16 - strlen($this->getCardparts()); $i++) {
                $card .= "*";
            }
            $card .= $this->getCardparts();
        }
     ...
     * @return string
     */
    public function getExpireProcessing()
    {
        if (is_null($this->getExpire())) return "";
        $expire = $this->getExpire();
        if (empty($expire) || strlen($expire) < 4) return "";
        return substr($expire, 0, 2) . "月 " . substr($expire, 2, 2) . "年";
    }
     ...
     * @return string
     */
    public function getNextDateStr($format = 'Ymd')
    {
        if (!$this->status) return "";
        if ($this->next_date instanceof \DateTime) {
            $workDate = $this->next_date;
     ...
     * @return string
     */
    public function getStartDateStr($format = 'Ymd')
    {
        if ($this->start_date instanceof \DateTime) {
            $workDate = $this->start_date;
            $workDate->setTimeZone(new \DateTimeZone('Asia/Tokyo'));
            return $workDate->format($format);
        }
        return "";
     ...
     * @return string
     */
    public function getStopDateStr($format = 'Ymd')
    {
        if ($this->stop_date instanceof \DateTime) {
            $workDate = $this->stop_date;
            $workDate->setTimeZone(new \DateTimeZone('Asia/Tokyo'));
            return $workDate->format($format);
        }
        return "";
    ...
    /**
     * Add OrderResultCards
     *
     * @param  \Plugin\RemisePayment42\Entity\OrderResultCard $OrderResultCard
     * @return OrderResultCards
     */
    public function addOrderResultCard(\Plugin\RemisePayment42\Entity\OrderResultCard $OrderResultCard)
    {
        $this->OrderResultCards[] = $OrderResultCard;
        return $this;
    }

    /**
     * Remove OrderResultCards
     *
     * @param \Plugin\RemisePayment42\Entity\OrderResultCard $OrderResultCard
     */
    public function removeOrderResultCard(\Plugin\RemisePayment42\Entity\OrderResultCard $OrderResultCard)
    {
        $this->OrderResultCards->removeElement($OrderResultCard);
    }

    /**
     * Get OrderResultCards
     *
     * @return \Doctrine\Common\Collections\Collection
     */
    public function getOrderResultCards()
    {
        return $this->OrderResultCards;
    }
}
